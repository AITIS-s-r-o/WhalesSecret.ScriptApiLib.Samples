...
public class TickerBasic : IScriptApiSample
{
    ...
    public async Task RunSampleAsync(ExchangeMarket exchangeMarket)
    {
        ...
        Console.WriteLine($"Connect to {exchangeMarket} exchange with a public connection.").ConfigureAwait(false);
        ...
        Console.WriteLine($"Public connection to {exchangeMarket} has been established successfully.").ConfigureAwait(false);

        SymbolPair symbolPair = SymbolPair.BTC_USDT;
        Console.WriteLine($"Create subscription for '{symbolPair}' ticker on {exchangeMarket}.").ConfigureAwait(false);
        ...
        Console.WriteLine($"Ticker subscription for '{symbolPair}' on {exchangeMarket} has been created successfully as '{subscription}'.").ConfigureAwait(false);

        Console.WriteLine($"Wait for next 2 ticker updates for {symbolPair}.").ConfigureAwait(false);
        Ticker ticker = await subscription.GetNewerTickerAsync(timeoutCts.Token).ConfigureAwait(false);

        Console.WriteLine($"First ticker update '{ticker}' has been received.").ConfigureAwait(false);

        Ticker ticker2 = await subscription.GetNewerTickerAsync(timeoutCts.Token).ConfigureAwait(false);

        Console.WriteLine($"Second ticker update '{ticker2}' has been received.").ConfigureAwait(false);

        Console.WriteLine("Disposing ticker subscription, trade API client, and script API.").ConfigureAwait(false);
    }
}